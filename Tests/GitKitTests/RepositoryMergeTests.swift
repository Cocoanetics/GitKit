...
@Suite("Repository.merge")
struct RepositoryMergeTests {
    ...
    func alreadyUpToDate() throws {
        ...
        _ = try repo.merge(ref: "feature") // brings main up
        // Fresh handle for the second call, matching the old per-call
        // client, so it observes the fast-forwarded ref from disk.
        let again = try Repository.open(at: dir).merge(ref: "feature")
    ...
    func unknownRef() throws {
        ...
        let repo = try Repository.open(at: dir)
        #expect(throws: Libgit2Error.self) {
            _ = try repo.merge(ref: "nopebranch")
        }
    }
}
#endif
