...
/// Single entry returned by ``Repository/stashList()``.
...
/// Options for ``Repository/stashSave(message:author:flags:)``. Mirrors
/// the bits real `git stash push` exposes.
public struct StashSaveFlags: OptionSet, Sendable {
    /// Raw `GIT_STASH_*` bitmask handed to `git_stash_save`.
    public let rawValue: UInt32
    /// Create a flag set from a raw `GIT_STASH_*` bitmask.
    public init(rawValue: UInt32) { self.rawValue = rawValue }

    /// Plain `git stash push`: tracked changes are stashed and the
    /// working tree + index reset to `HEAD`.
    public static let `default`         = StashSaveFlags([])
    /// `GIT_STASH_KEEP_INDEX` — `git stash push --keep-index`: staged
    /// changes go into the stash but stay in the index too.
    public static let keepIndex         = StashSaveFlags(rawValue: UInt32(GIT_STASH_KEEP_INDEX.rawValue))
    /// `GIT_STASH_INCLUDE_UNTRACKED` — `git stash push
    /// --include-untracked`: untracked files are stashed and removed
    /// from the working tree.
    public static let includeUntracked  = StashSaveFlags(rawValue: UInt32(GIT_STASH_INCLUDE_UNTRACKED.rawValue))
    /// `GIT_STASH_INCLUDE_IGNORED` — the ignored-files half of
    /// `git stash push --all`: ignored files are stashed and removed
    /// from the working tree.
...
extension Repository {
    ...
    public func stashApply(index: Int, reinstateIndex: Bool = false) throws {
    ...
    public func stashPop(index: Int, reinstateIndex: Bool = false) throws {
        var opts = git_stash_apply_options()
        try check(git_stash_apply_options_init(
            &opts, UInt32(GIT_STASH_APPLY_OPTIONS_VERSION)))
        if reinstateIndex {
    ...
    public func stashList() throws -> [Libgit2StashEntry] {
    ...
    public func stashClear() throws {
        let entries = try stashList()
        for entry in entries.sorted(by: { $0.index < $1.index }).reversed() {
            try stashDrop(index: entry.index)
        }
    }
    ...
    public func stashShow(index: Int) throws -> (filesChanged: Int, insertions: Int, deletions: Int) {
    ...
    public func stashBranch(name: String, index: Int = 0) throws {
