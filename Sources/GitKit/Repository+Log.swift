...
/// One commit returned by ``Repository/log(query:)``.
public struct LogEntry: Sendable {
    /// Full 40-char commit SHA (what `%H` resolves to).
    public let sha: String
    /// First 7 characters of ``sha`` (what `%h` resolves to).
    public let shortSHA: String
    /// Full SHA of the commit's tree object (what `%T` resolves to).
    public let treeSHA: String
    /// Full SHAs of the parents in storage order — first parent first
    /// (what `%P` resolves to). Empty for a root commit, 2+ for a merge.
    public let parentSHAs: [String]

    /// Author name (what `%an` resolves to).
    public let authorName: String
    /// Author email without the `<>` brackets (what `%ae` resolves to).
    public let authorEmail: String
    /// Author timestamp in Unix seconds (what `%at` resolves to).
    public let authorTime: TimeInterval
    /// Author timezone as minutes east of UTC (e.g. 120 = +0200).
    public let authorOffsetMinutes: Int

    /// Committer name (what `%cn` resolves to).
    public let committerName: String
    /// Committer email without the `<>` brackets (what `%ce` resolves to).
    public let committerEmail: String
    /// Committer timestamp in Unix seconds (what `%ct` resolves to).
    public let committerTime: TimeInterval
    /// Committer timezone as minutes east of UTC (e.g. 120 = +0200).
    public let committerOffsetMinutes: Int

    /// Raw commit message — subject, blank separator line, body —
    /// including the trailing newline as stored in the object.
    ...
    /// True for commits with more than one parent — what triggers the
    /// `Merge:` line in default `git log` output.
...
public struct LogQuery: Sendable {
    ...
    /// Creates a query. The defaults reproduce bare `git log`: walk
    /// from HEAD, exclude nothing, no limit, no path filter.
    public init(
        starts: [String] = [],
        excludes: [String] = [],
        maxCount: Int? = nil,
...
extension Repository {
    ...
    public func log(_ query: LogQuery = LogQuery()) throws -> [LogEntry] {
