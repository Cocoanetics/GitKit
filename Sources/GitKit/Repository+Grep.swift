...
public struct GrepMatch: Sendable, Equatable {
    ...
    /// Memberwise init — `grep` builds these for you, but hosts can
    /// fabricate matches too (test fixtures, merged result sets).
...
extension Repository {
    ...
    public func grep(
        ...
        let workdir = try requireWorkdir()

        var paths = try indexedPaths()
        if includeUntracked {
    ...
    ///
    /// Public so embedders can match pathspecs with exactly the
    /// semantics ``grep(pattern:options:pathFilters:includeUntracked:)``
    /// applies.
    public static func glob(pattern: String, name: String) -> Bool {
        let p = Array(pattern)
        let n = Array(name)
        var memo: [[Bool?]] = Array(
