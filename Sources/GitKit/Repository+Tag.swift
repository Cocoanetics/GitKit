...
/// One entry returned by ``Repository/tagList(pattern:)``. Annotated
...
public struct TagEntry: Sendable, Equatable {
    ...
    /// True for annotated tags (a tag object with tagger + message),
    /// false for lightweight refs pointing straight at a commit.
...
extension Repository {
    ...
    public func tagList(pattern: String? = nil) throws -> [String] {
    ...
    public func tagDetails(pattern: String? = nil) throws -> [TagEntry] {
        let names = try tagList(pattern: pattern)
    ...
    public func tagCreateAnnotated(
        ...
        message: String, tagger: Signature? = nil,
        force: Bool = false,
        ...
        let sig = try SignatureResolver.resolve(
            role: .committer, override: tagger, repo: repo, env: env)
    ...
    @discardableResult
    public func tagDelete(name: String) throws -> String {
    ...
    public func tagExists(_ name: String) throws -> Bool {
