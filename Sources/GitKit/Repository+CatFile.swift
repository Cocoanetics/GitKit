...
public struct ObjectMetadata: Sendable {
    /// The four git object types. `rawValue` matches what
    /// `cat-file -t` prints.
    ...
    /// Type of the resolved object. Annotated tags report `.tag` —
    /// append `^{}` to the revspec to peel, as in real git.
...
extension Repository {
    ...
    /// `git cat-file -t -s`.
    public func objectMetadata(of revspec: String) throws -> ObjectMetadata {
    ...
    public func catFileBlob(_ revspec: String) throws -> Data {
        var obj: OpaquePointer?
        try check(revspec.withCString { name in
            git_revparse_single(&obj, repo, name)
        })
