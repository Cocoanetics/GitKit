...
extension Repository {
    ...
    ///
    /// `env` feeds the real-git committer identity precedence chain
    /// (`GIT_COMMITTER_*`); see ``SignatureResolver``.
    @discardableResult
    ...
    public func cherryPickSkip() throws {
        try cherryPickAbort()
    }
