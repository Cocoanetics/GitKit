...
public struct StatusEntry: Sendable, Equatable {
    ...
    /// Pre-rename path when `indexState`/`workdirState` is `.renamed`,
    /// nil otherwise.
    public let oldPath: String?

    /// HEAD ↔ index change — the staged side, `X` in short format.
    public let indexState: ChangeKind
    /// Index ↔ working-tree change — the unstaged side, `Y` in short format.
    public let workdirState: ChangeKind
    /// True for files not in the index — short format's `??`.
    public let isUntracked: Bool
    /// True when a `.gitignore` rule matches the path. Always false
    /// for ``Repository/status()`` entries (it skips ignored files).
    public let isIgnored: Bool
    /// True for unmerged paths (merge conflicts) — short format's `UU`.
    public let isConflicted: Bool

    /// What changed on one side of the comparison (HEAD ↔ index or
    /// index ↔ working tree).
    public enum ChangeKind: Sendable, Equatable {
        /// No change on this side — a space in the short-format column.
        case unchanged
        /// File added (`A`).
        case newFile
        /// Content changed (`M`).
        ...
        /// File removed (`D`).
        case deleted
        /// Rename detected (`R`); `StatusEntry.oldPath` holds the source.
        case renamed
        /// File type flipped, e.g. regular file ↔ symlink (`T`).
        case typeChange
...
/// Result of ``Repository/status()``. The CLI bins these into the
...
public struct StatusReport: Sendable {
    ...
    /// Commits `upstreamRef` has that the local branch lacks, nil
    /// when no upstream is configured.
    public let behind: Int?
    /// Every changed path libgit2 reported — staged, unstaged,
    /// untracked and conflicted alike; ignored files are excluded.
...
extension Repository {
    ...
    public func status() throws -> StatusReport {
